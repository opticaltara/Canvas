...
import { Play, Trash, Edit, Check, X, ChevronRight, ChevronDown, Link, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
interface EnhancedCellComponentProps {
  cell: Cell;
  executeCell: () => void;
  autoExecuteEnabled?: boolean;
}

export default function EnhancedCellComponent({ 
  cell, 
  executeCell,
  autoExecuteEnabled = false,
}: EnhancedCellComponentProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [editValue, setEditValue] = useState(cell.content);
  const [showDependencies, setShowDependencies] = useState(false);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [suggestedDependencies, setSuggestedDependencies] = useState<string[]>([]);
  const { 
    updateCell, 
    deleteCell, 
    currentNotebook, 
  ...
  // Track if this is a temporary auto-save
  const autoSaveTimerRef = useRef<NodeJS.Timeout | null>(null);
  const [hasPendingChanges, setHasPendingChanges] = useState(false);
  
  // Get suggested dependencies whenever the cell content changes
  useEffect(() => {
    if (cell && cell.id && !isEditing) {
      const deps = suggestDependencies(cell.id);
      setSuggestedDependencies(
        // Filter out dependencies that are already added
        deps.filter(dep => !cell.dependencies.includes(dep))
      );
      
      // Show suggestions if there are any new ones
      if (deps.length > 0 && deps.some(dep => !cell.dependencies.includes(dep))) {
        setShowSuggestions(true);
      }
    }
  }, [cell.content, cell.id, cell.dependencies, suggestDependencies, isEditing]);
  
  // Auto-execute the cell if enabled and not editing
  useEffect(() => {
    if (autoExecuteEnabled && !isEditing && cell.status === CellStatus.IDLE) {
      executeCell();
    }
  }, [autoExecuteEnabled, cell.status, executeCell, isEditing]);
  
  // Setup auto-save during editing
  useEffect(() => {
    if (isEditing) {
  ...
  }, [isEditing, editValue, cell.content, cell.id, updateCell]);

  const handleSave = () => {
    updateCell(cell.id, editValue, autoExecuteEnabled);
    setIsEditing(false);
              ...
              {availableCells.map(c => (
                <button
                  ...
                  className={`
                    ...
                    ${suggestedDependencies.includes(c.id) ? 'border-2 border-green-500' : ''}
                  `}
                  title={
                    cell.dependencies.includes(c.id) 
                      ? 'Already a dependency' 
                      : suggestedDependencies.includes(c.id)
  ...
  return (
    ...
    >
      ...
      {/* Suggested dependencies notice */}
      {showSuggestions && suggestedDependencies.length > 0 && !isEditing && (
        <div className="mb-2 p-2 bg-green-50 dark:bg-green-900 dark:bg-opacity-20 border border-green-200 dark:border-green-800 rounded flex items-start">
          <AlertCircle className="w-4 h-4 text-green-500 mr-2 mt-0.5" />
          <div>
            <div className="text-sm font-medium text-green-800 dark:text-green-300">
              Suggested Dependencies
            </div>
            <div className="text-xs text-green-700 dark:text-green-400">
              We detected that this cell might depend on {suggestedDependencies.length} other cell(s).
              <button
      ...
      <div className="cell-content relative">
        {hasPendingChanges && !isEditing && (
          <div className="absolute right-0 top-0 text-xs text-blue-500 dark:text-blue-400 p-1 bg-white dark:bg-gray-800 rounded opacity-75">
            Auto-saved
          </div>
        )}
            ...
            {cell.type !== CellType.MARKDOWN && (
              <div className="mb-2">
                <input
                  ...
                  value={cell.metadata?.description || ''}
                  onChange={(e) => {
                    const newCell = {
                      ...cell,
                      metadata: {
      ...
      <div className="cell-footer mt-4 pt-2 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center text-sm text-gray-500">
        ...
        >
          ...
          {suggestedDependencies.length > 0 && !showDependencies && (
            <span className="ml-1 text-green-500">+{suggestedDependencies.length} suggested</span>
          )}
        </button>
      </div>
