...
import RunbookHeader from "./components/RunbookHeader"
...
const InteractiveRunbookEnhanced: React.FC = () => {
  // Update the runbook name
  const [runbookName, setRunbookName] = useState("Payments Failure Investigation")
...
print("iOS errors: 800; Android errors: 200; Web errors: 500")`,
  ...
  // Update the runbooks array to reflect the new name
  ...
  const [currentPage, setCurrentPage] = useState("runbook")
  ...
  const handleAddRunbook = (name: string) => {
    const newRunbook: Runbook = { id: String(runbooks.length + 1), name, content: [] }
    setRunbooks([...runbooks, newRunbook])
  }

  const handleSelectRunbook = (id: string) => {
    const selectedRunbook = runbooks.find((runbook) => runbook.id === id)
    if (selectedRunbook) {
      setRunbookName(selectedRunbook.name)
      setCells(selectedRunbook.content)
      setCurrentPage("runbook")
    }
  }
  ...
  const handleRunbookNameChange = (newName: string) => {
    setRunbookName(newName)
  }
  ...
  // Add a new handler function for sharing to Slack
  const handleShareToSlack = (index: number) => {
    const cell = cells[index]
    console.log(`Sharing cell ${index} to Slack: ${cell.type}`)

    // In a real implementation, this would open a modal or make an API call to share to Slack
    // For now, we'll just show an alert
    alert(`Cell "${cell.type}" has been shared to Slack!`)
  }
    ...
    <div className="h-screen flex flex-col">
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {currentPage === "runbook" ? (
            <>
              <RunbookHeader name={runbookName} onNameChange={handleRunbookNameChange} />
                    ...
                    {cells.map((cell, index) => (
                      <React.Fragment key={index}>
                        {cell.question && cell.answer && (
                          <QuestionAnswerUI question={cell.question} answer={cell.answer} />
                        )}
                        <Cell
                          {...cell}
                          id={`cell-${index}`}
                          ...
                          onShareToSlack={() => handleShareToSlack(index)}
...
export default InteractiveRunbookEnhanced
